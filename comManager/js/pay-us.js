...
var submitDataToCheckout = () =>{
    if(formInfo.reportValidity()){
        $('#buttonData').html(`
        <i class="fa fa-credit-card"></i> Continue Checkout <img src='../../uploads/loadpay.svg' width="5%" />
            `);

            var submitData = fetch('../../Pay/pay-process.php',{
            method: 'POST',
            body: formData,
            cache: 'no-cache'
                        ...
                        if (result.error) {
                            ...
                            if (result.paymentIntent.status === 'succeeded') {
                                ...
                                var cartAction = fetch('../../Cart/actionCart.php').then(function(response){
                                    ...
                                    formDataToSucceeded.append('msg', 'successUS');
                                    //Fetch the cart
                                    fetch('./redirect.php',{
                                        method: 'POST',
